...
class MedicalReportView(ctk.CTkFrame):
    def __init__(self, master, patient, ex, on_back):
        ...
        add_info(info_grid, "Пациент", patient['username'])
        add_info(info_grid, "Дата осмотра", ex['date'])
        add_info(info_grid, "Диагноз", ex.get('diagnosis', 'Плановый'))

        # СЕТКА МЕТРИК (То, чего не хватало)
